    ...
    <tr id="componentattachmentrow${loop.count}" class="tr_clone">
        ...
        <td colspan="6">
            ...
            <input class="toplabelledInput" id="comp_filecomment${loop.count}"
                   name="<portlet:namespace/><%=Release._Fields.ATTACHMENTS%><%=Attachment._Fields.COMMENT%>"
                   type="text" placeholder="Enter comments"
                   value="<sw360:out value="${attachment.comment}"/>"
                    />
        </td>
        <td class="downloader">
        ...
        <td class="deletor">
            <img src="<%=request.getContextPath()%>/images/Trash.png"
                 ...
                 alt="Delete">
                <%--<input type="button" onclick="deleteAttachment('componentattachmentrow${loop.count}','${attachment.id}')" value="Delete"/>--%>
        </td>
    </tr>
</core_rt:forEach>
